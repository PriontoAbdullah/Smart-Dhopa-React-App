import { faCloudUploadAlt, faMinus, faPencilAlt, faPlus, faTrashAlt } from '@fortawesome/free-solid-svg-icons';
...
const AllOrders = () => {
	...
	const openOrderModal = (orderId) => {
		setModalIsOpen(true);
		const selectedOrder = ContextData.order.find((item) => item._id === orderId);
	...
	const openDataDeleteModal = (currentProduct) => {
		setDeleteModal(true);
		const reamingProducts = selectOrder.products.filter((item) => item.id !== currentProduct.id);
					...
					{selectOrder && (
						<div className="px-5 my-3 single-order">
							<form className="" onSubmit={handleSubmit(onSubmit)}>
								<div className="">
									<h3 className="text-info text-center mt-4 mb-2">Customer Details</h3>
									<button
										type="button"
								...
								<Table>
									...
									<tbody>
										<tr>
											...
											<td className="table-data">{selectOrder.shipment.fullName}</td>
											<td className="table-data">{selectOrder.shipment.email}</td>
											<td className="table-data">{selectOrder.shipment.mobileNumber}</td>
										</tr>
									</tbody>
								</Table>
								...
								<div className="form-group text-center ">
									<input
										...
										name="id"
									/>
									<button type="submit" className="btn btn-amber px-5 table-data">
										...
										<tr key={item.id}>
											...
											<td className="text-center">
												<div className="">
													...
													<span className="quantity btnQ"> {item.quantity}</span>
													<button
